...
namespace ArdupilotMega.Devices
{
	public partial class HIDDevice : USBDevice
	{
		...
		///  </returns>
		///  
		public HIDDevice(string vid, string pid) : base(vid, pid)
		{
			OpenHandle(this.devPath, true);
		}
		...
		internal Boolean GetAttributes(SafeFileHandle hidHandle, ref HIDD_ATTRIBUTES deviceAttributes)
		{
			Boolean success;
			try
			{ 
		...
		internal async Task<int> GetInputReportViaInterruptTransfer(FileStream fileStreamDeviceData,  Byte[] inputReportBuffer, CancellationTokenSource cts)
			{
				...
				try
				{
					...
					if (fileStreamDeviceData.CanRead)
					{
						...
						Task<int> t = fileStreamDeviceData.ReadAsync(inputReportBuffer, 0, inputReportBuffer.Length, cts.Token);

						int bytesRead = await t;
			...
			internal async Task<int> SendOutputReportViaInterruptTransfer(FileStream fileStreamDeviceData, SafeFileHandle hidHandle, Byte[] outputReportBuffer, CancellationTokenSource cts)
			{
		...
		internal SafeFileHandle OpenHandle(String devicePathName, Boolean readAndWrite)
		{
			...
			{ 
				if (readAndWrite)
				{
					...
					hidHandle = CreateFile(devicePathName, (uint)(GenericAccessRights.GenericRead | GenericAccessRights.GenericWrite), (uint)(FileHandleShareMode.FileShareRead | FileHandleShareMode.FileShareWrite), IntPtr.Zero, (uint)FileCreationDisposition.OpenExisting, 0, IntPtr.Zero);
				}
				else
				{
					hidHandle = CreateFile(devicePathName, 0, (uint)(FileHandleShareMode.FileShareRead | FileHandleShareMode.FileShareWrite), IntPtr.Zero, (uint)FileCreationDisposition.OpenExisting, 0, IntPtr.Zero);
				}
			}
			catch (Exception ex)
			{
